...
import traci  # <--- SUMO integration added
import twilio
...
# === SUMO Setup ===
SUMO_CONFIG_PATH = "map.sumocfg"  # Replace with your .sumocfg path
JUNCTION_ID = "n1"  # Replace with your traffic light ID
...
def control_traffic_light(vehicles_left, vehicles_right):
    ...
    if total_vehicles > VEHICLE_COUNT_THRESHOLD:
        try:
            print("🚦 High vehicle count: Changing traffic signal to allow flow")
            traci.trafficlight.setRedYellowGreenState(JUNCTION_ID, "GGrrr")  # Adjust pattern as per SUMO logic
        except Exception as e:
            print(f"❌ Error changing traffic light: {e}")


from twilio.rest import Client

# Your Twilio credentials (replace with env vars in production!)
TWILIO_ACCOUNT_SID = "AC3e9621790fea1070b200fd40a16c1191"
TWILIO_AUTH_TOKEN = "d25bc282410c2247ac6f71f2b55c1cdf"
TWILIO_PHONE_NUMBER = "6284710074"  # Replace with your Twilio number
POLICE_NUMBER = ""              # Replace with the target number

def send_sms_alert(number, message):
    print(f"📲 ALERT: Sending SMS to {number}: {message}")
    try:
...
def check_violence_alert(current_violence_alert):
    ...
    if current_violence_alert:
        ...
        elif time.time() - violence_detected_start > VIOLENCE_ALERT_DURATION and not violence_alert_sent:
            send_sms_alert("9877035742", "Violence detected continuously for over 100 seconds.")
...
def check_animal_duration(current_animal_alert):
    ...
    if current_animal_alert:
        ...
        elif time.time() - animal_detected_start > ANIMAL_ALERT_DURATION and not animal_light_adjusted:
...
def adjust_traffic_light_for_animals():
    try:
        print("🦘 Animal detected > 20s. Extending green phase.")
        current_phase = traci.trafficlight.getPhase("n1")
        traci.trafficlight.setPhaseDuration("n1", 60)
        print(f"✅ Phase {current_phase} extended to 60s")
    except Exception as e:
        print(f"❌ Failed to adjust traffic light for animals: {e}")



def process_frame(vehicles_left, vehicles_right, current_violence_alert, current_animal_alert):
    try:
        if traci.isLoaded():
            traci.simulationStep()
            control_traffic_light(vehicles_left, vehicles_right)
            check_violence_alert(current_violence_alert)
            check_animal_duration(current_animal_alert)
    except Exception as e:
        print(f"⚠️ Error during SUMO frame step: {e}")
    ...
    try:
        while True:
            ...
            if not paused:
                ...
                process_frame(vehicles_left, vehicles_right, analyzer.current_violence_alert, analyzer.current_animal_alert)
