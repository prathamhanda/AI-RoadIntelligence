...
def main():
    ...
    # Calculate display scale to fit screen (more conservative sizing for Windows)
    max_display_width = 1024  # Smaller for better Windows compatibility
    ...
    vertices1 = np.array([(2408, 733), (3810, 1920), (2222, 2130), (2052, 744), (2402, 735)], dtype=np.int32)
    vertices2 = np.array([(2020, 752), (1948, 2138), (165, 2135), (1702, 760), (2018, 752)], dtype=np.int32)
    lane_threshold = 2074
    x1, x2 = 683, 2160  # Detection area
    ...
    # Create the display window BEFORE processing starts
    window_name = 'Indian Traffic Analysis - Live Feed'
    ...
    print(f"Display window created: {display_width}x{display_height}")
    print("Processing... Press 'q' to quit, 'p' to pause/unpause, 's' to step frame by frame, SPACE to continue")
    print("If you don't see the window, check your taskbar or press Alt+Tab")
    
    frame_count = 0
    paused = False
    
    # Add a small delay to ensure window is created
    time.sleep(1)
    
    # Show initial message in window
    initial_frame = np.zeros((display_height, display_width, 3), dtype=np.uint8)
    cv2.putText(initial_frame, "Starting video processing...", (50, display_height//2), 
                cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    cv2.putText(initial_frame, "Press any key to continue", (50, display_height//2 + 50), 
                cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    cv2.imshow(window_name, initial_frame)
    cv2.waitKey(2000)  # Wait 2 seconds or until key press
